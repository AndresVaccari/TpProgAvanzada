...
public class PanelAdmin {
	public void actualizarPanel(HttpServletRequest request,
		...
		try {
			...
			ResultSet contadorPentientes = stContadorPendientes.executeQuery(
					"SELECT COUNT(*) FROM usuario WHERE Estado = 0");
			ResultSet usuariosPendientes = stPendientes.executeQuery(
					"SELECT usuario FROM usuario WHERE Estado = 0");

			Statement stContadorActivos = conn.createStatement();
			Statement stActivos = conn.createStatement();
			ResultSet contadorActivos = stContadorActivos.executeQuery(
					"SELECT COUNT(*) FROM usuario WHERE Estado = 1");
			ResultSet usuariosActivos = stActivos.executeQuery(
					"SELECT usuario FROM usuario WHERE Estado = 1");

			Statement stContadorInactivos = conn.createStatement();
			Statement stInactivos = conn.createStatement();
			ResultSet contadorInactivos = stContadorInactivos.executeQuery(
					"SELECT COUNT(*) FROM usuario WHERE Estado = 2");
			ResultSet usuariosInactivos = stInactivos.executeQuery(
					"SELECT usuario FROM usuario WHERE Estado = 2");
			...
			String[] usuariosInactivosLista = new String[inactivos];
